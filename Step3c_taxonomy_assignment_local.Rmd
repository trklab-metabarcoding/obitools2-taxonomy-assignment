...
params:
    ...
    step_3a_analysis_date: "20241119" #YYYYMMDD
    step_3c_analysis_date: "20240530" #YYYYMMDD
    step_2b_analysis_date: "20241119_hhoff" #YYYYMMDD_[initials]
...
## Merge the two tabulated files together. 1) ref_seq (the output from step 2b) and 2) the tabulated file that you created above with the header "Create output for use in phyloseq." 
```{r}
ref_seq_tab_2b <- (glue("{output_path}/{step_2b_analysis_date}_ref_seq.tab"))
ref_seq_tab_2b <- read.table(ref_seq_tab_2c,  sep="\t", header = TRUE)
head(ref_seq_tab_2c)

output_tab_3c <- (glue("{output_path}/{DATE}_local_alluniq.clean.tag.ann.tab"))
...
write.table(merged_3c_for_phyloseq, "{DATE}_local_alluniq.clean.tag.ann.tab.headers")
```
