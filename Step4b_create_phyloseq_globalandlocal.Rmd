...
pacman::p_load(knitr, here, tidyr, dplyr, vegan, vegetarian, scales, datawizard, ggplot2, ggrepel, glue, textshape, stringr)
...
# filter for new percent match if not in local already
...
```{r}
print(paste0("The mean depth across all taxa for the local keep subset: ", mean(local_keep$count)))
print(paste0("The mean depth across all taxa for the global keep subset: ", mean(global_keep$count))) 
print(paste0("The median depth across all taxa for the local keep subset: ", median(local_keep$count))) 
print(paste0("The median depth across all taxa for the global keep subset: ", median(global_keep$count)))
...
# x-fold difference in local over global
## This assumes your local has better median read depth than the global. Some projects may still have a small local database relative to global, so you could switch global and local in the division, and make sure to report that the global is x-fold greater than local.
...
# total number of reads in each set
...
```{r warning=FALSE}
local_nontrace_global_stats <- as.data.frame(cbind(tapply(local_nontrace_global_depth, paste(data_samples[[params$minor_grouping]], data_samples[[params$major_grouping]]), mean), tapply(local_nontrace_global_depth, paste(data_samples[[params$minor_grouping]], data_samples[[params$major_grouping]]), sd), tapply(local_nontrace_global_depth, paste(data_samples[[params$minor_grouping]], data_samples[[params$major_grouping]]), function(x) length(x))))

colnames(local_nontrace_global_stats) <- c("mean","sd", "length")
...
#Set the index of the samples data to sample names; Phyloseq needs this to match up teh different shapes of OTU, TAX, and Samples tables
