...
```{r}
input_path <- file.path(params$data_path, params$project_code, "merged_runs", sprintf("%s_%s", params$step_3b_analysis_date, params$user))
...
```{r}
# mean for global counts
print(paste0("The mean counts across all taxa for the global keep set: ", round(mean(global_keep$count))))

# median for global counts
print(paste0("The median counts across all taxa for the global keep set: ", round(median(global_keep$count))))

# maximum for global counts
print(paste0("The maximum counts for any one taxa in the global keep set: ", max(global_keep$count)))

# total number of reads across taxa
