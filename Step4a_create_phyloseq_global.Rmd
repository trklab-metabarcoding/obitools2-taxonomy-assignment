...
params:
    ...
    min_read_count: 1000 # default for catching any cleaned files with low read counts
...
pacman::p_load(knitr, here, tidyr, dplyr, vegan, glue, scales, datawizard) # this takes a few minutes
...
filter_low_count_samples <- function(df) {
  ...
  sample_sums <- df %>% 
    select(starts_with(c("sample.", "sample_marker."))) %>% 
  ...
  non_sample_cols <- names(df)[!grepl("^sample\\.|^sample_marker", names(df))]
...
##TODO (TJD): Update the metadata (data_samples) to remove rows that get filtered out at this step.
...
#### Median, mean, and max for global counts
```{r}
# Mean read depth of taxa identified using the global reference library
print(paste0("The mean depth across all taxa for the global keep set: ", round(mean(global_keep$count))))

# Median read depth of taxa identified using the global reference library
print(paste0("The median depth across all taxa for the global keep set: ", round(median(global_keep$count))))

# Maximum read depth of taxa identified using the global reference library
print(paste0("The maximum depth for any one taxa in the global keep set: ", max(global_keep$count)))

# Overall read depth of taxa identified using the global reference library
...
```{r}
OTU_table <- global_keep[,grep("sample\\|sample_marker\\", colnames(global_keep))]
names(OTU_table) <- gsub(pattern = "sample.|sample_marker.", replacement = "", x = names(OTU_table))
...
```{r}
# Calculate depth by sample
global_depth <- colSums(OTU_table)

# Clean up sample IDs - remove the "_Sx"
depth_data <- tibble(
  SampleID = names(global_depth),
  global_depth = global_depth
) %>%
  mutate(SampleID = str_remove(SampleID, "_S\\d+$"))

# Join with your data_samples
...
global_stats <- combined_data %>%
  ...
  summarize(
    mean = mean(global_depth, na.rm = TRUE),
    sd = sd(global_depth, na.rm = TRUE),
    count = n(),
    .groups = "drop"
  )
global_stats
...
```{r}
tax_table <- global_keep[,-grep("sample\\|sample_marker", colnames(global_keep))]
...
TAX <- phyloseq::tax_table(tax_table2)

if (identical(phyloseq::sample_names(OTU), phyloseq::sample_names(samples))){
...
```{r}
rds_file<- phyloseq::phyloseq(OTU, TAX, samples) 
