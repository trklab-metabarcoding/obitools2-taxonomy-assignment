...
pacman::p_load(knitr, here, tidyr, dplyr, janitor)
...
```{r}
file_list <- list.files(path=file.path(params$data_path, params$project_code, params$seq_run_name),
           recursive=T,
           pattern=("*.fastq"),
           full.names=T)
merged_data <- grep("results/id_data", file_list, value = TRUE, fixed = TRUE)
```
...
samples <- as.data.frame(str_extract(files, "[^_]+"))
