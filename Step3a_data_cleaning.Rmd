...
After this has run, look at the `tabseqs_calcs` data frame for your results. The results are also passed out as file to your output path with extension `"calcs.tab"`.
```{r}

tabseqs <- read_tsv(list.files(output_path, pattern = "_alluniq.clean.tab", full.names=T))
sample_columns <- tabseqs[, grepl("^sample:", names(tabseqs))]
sample_means <- colMeans(sample_columns)
sample_ranges <- apply(sample_columns, 2, range)
tabseqs_calcs <- data.frame(
  Sample = names(sample_columns),
  Mean = sample_means,
  Min = sample_ranges[1, ],
  Max = sample_ranges[2, ]
)
write_tsv(tabseqs_calcs, glue("{output_path}/{params$step_3a_analysis_date}_alluniq.clean.calcs.tab"))
```
...
```{r}
file.copy("./Step3a_data_cleaning.html", file.path(glue("{output_path}/Step3a_data_cleaning.html")))
```