...
subtitle: "Assign taxonomy to cleaned sequences using global database file."
...
params:
    ...
    step_3b_analysis_date: "20240530" #YYYYMMDD
    project_code: "test"
    taxa_division: "PLN"
    locus: "trnL"
    region_of_interest: "P6"
    latest_db_date: "20240430" #look in `ref_lib_path/taxa_division/locus/region_of_interest` for the most recent global reference library you want to use
---
...
pacman::p_load(knitr, here, tidyr, dplyr, readxl, stringr, spgs, filesstrings, glue)
...
global_ref_db <- file.path(params$ref_lib_path, params$taxa_division, params$locus, params$region_of_interest, params$latest_db_date, sprintf("%s_ecoPCRformat", params$latest_db_date))
# -d in ecotag; ecoPCR taxonomy Database name.

ref_seq <- file.path(params$ref_lib_path, params$taxa_division, params$locus, params$region_of_interest, params$latest_db_date, sprintf("%s_%s_completeDB.fasta", params$latest_db_date, params$region_of_interest))
...
Sys.setenv(DATE=params$step_3b_analysis_date)
...
# Note: you will likely get a red "error" message that obitools can't find the .pdx file. This message can be ignored and this step should carry on running as expected.  
...
```{r}
file.copy("./Step3b_taxonomy_assignment_global.nb.html", file.path(glue("{output_path}/Step3b_taxonomy_assignment_global.nb.html")), overwrite = TRUE)
```
